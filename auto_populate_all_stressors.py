...
def parse_markdown_stressor(markdown_text):
    ...
    # Impact function name is added LATER, once the ecosystem is known.

    return stressor_dict


def create_ecosystem_stressor_file(ecosystem, stressors_data, stressor_dir="ecosystem_stressors"):
    """Creates or overwrites a Python file for an ecosystem's stressors."""

    filename = ecosystem.lower().replace(" ", "_").replace("(", "").replace(")", "") + "_stressors.py"
    filepath = os.path.join(stressor_dir, filename)

    with open(filepath, "w") as f:
        f.write("from stressor_templates import *\n")
        f.write("import copy\n\n")
        f.write(f"{ecosystem.lower().replace(' ', '_').replace('(', '').replace(')', '')}_stressors = {{\n")

        for stressor_name, stressor_info in stressors_data.items():
            template_name = None
            # Determine which template to use (same logic as before).
            ...
            if template_name:
                ...
                f.write("{},\n")  # If no template is found, create an empty dictionary

        f.write("}\n\n")

        # Add the specific details for each stressor
        for stressor_name, stressor_info in stressors_data.items():
            f.write(f'# --- {stressor_name} ---\n')
            for key, value in stressor_info.items():
                if key == "Impact Function":
                     # NOW we add the ecosystem to the impact function name
                    ...
                    f.write(f'{ecosystem.lower().replace(" ", "_").replace("(", "").replace(")", "")}_stressors["{stressor_name}"]["{key}"] = "{stressor_func_name}"\n')
                elif key not in ["Stressor Name", "Ecosystem"]:
                    f.write(f'{ecosystem.lower().replace(" ", "_").replace("(", "").replace(")", "")}_stressors["{stressor_name}"]["{key}"] = {repr(value)}\n')
            f.write("\n")
...
def process_markdown_files(docs_dir="docs", stressor_dir="ecosystem_stressors"):
    ...
    for filename in os.listdir(docs_dir):
        if filename.endswith(".md") and filename.startswith("Stressors_"):
            ...
            # --- Extract Ecosystem Name ---
            # 1. Remove "Stressors_" prefix and ".md" suffix
            ecosystem_name_raw = filename[len("Stressors_"):-3]
            # 2. Split by underscores
            name_parts = ecosystem_name_raw.split("_")
            # 3. Join with spaces to get the ecosystem name
            ecosystem = " ".join(name_parts)


            # 2. Split by Stressor Heading (###) ONLY
            stressor_sections = re.split(r"(?m)^### Stressor:", markdown_content)
            stressor_sections = [s for s in stressor_sections if s.strip()]
            stressors_data = {}  # Accumulate stressors for the *current* ecosystem

            for stressor_section in stressor_sections:
                # 3. Parse Stressor Information
                stressor_info = parse_markdown_stressor("### Stressor:" + stressor_section)  # Add back the header
                if stressor_info:
                    stressors_data[stressor_info["Stressor Name"]] = stressor_info

            # 4. Create/Populate the Python File (using the FILENAME-derived ecosystem)
            create_ecosystem_stressor_file(ecosystem, stressors_data, stressor_dir)
